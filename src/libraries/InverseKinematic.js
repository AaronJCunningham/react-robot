...
function addArrow(name, from, to, color = 0xffff00, length = 10) {
  ...
  // toPoint.normalize()
  arrows[name] = new THREE.ArrowHelper(
    toPoint.sub(origin).normalize(),
    origin,
    length,
    color,
    2,
...
function addVectorArrow(name, from, vector, color, length) {
  addArrow(
    name,
    from,
    [from[0] + vector[0], from[1] + vector[1], from[2] + vector[2]],
    color,
    length
  );
}
...
class InverseKinematic {
  ...
  calculateAngles(x, y, z, a, b, c, angles, config = [false, false, false]) {
    ...
    const alphaR0 = Math.asin(
      this.J_initial_absolute[4][1] / this.length2(J[4][1], J[4][0])
    );
    ...
    J[1][0] =
      Math.cos(R[0]) * this.geometry[0][0] +
      Math.sin(R[0]) * -this.geometry[0][1];
    J[1][1] =
      Math.sin(R[0]) * this.geometry[0][0] +
      Math.cos(R[0]) * this.geometry[0][1];
    ...
    const J1J4_projected_length_square =
      Math.pow(J4_x_z[0] - this.J_initial_absolute[1][0], 2) +
      Math.pow(J4_x_z[2] - this.J_initial_absolute[1][2], 2); // not using Math.sqrt
    ...
    R[1] -= Math.atan2(
      ...
      J4_x_z[0] - this.J_initial_absolute[1][0]
    ); // a''
    R[1] +=
      ((config[1] ? !config[0] : config[0]) ? 1.0 : -1.0) *
      Math.acos(
        (J1J4_projected_length_square -
          Math.pow(J2J4_length_x_z, 2) +
          Math.pow(this.V1_length_x_z, 2)) /
          (2.0 * J1J4_projected_length * this.V1_length_x_z)
      ); // a
